# PM5020 Power Meter: Scope and Burst Mode SCPI Example
This example shows how to connect to the PM5020 power meter and measure the power in software-triggered scope mode and external-triggered burst mode in SCPI commands. The basic CW mode example is also included. 
...
* Buffer & Speed: The CW mode transfers individual data points sequentially. The maximum sampling rate is approximately 1 kHz for a single channel, which is restricted by bus communication latency and data transfer overhead.
...
* Buffer & Speed: The scope mode stores up to 10000 samples with given averaging at max 100 kHz in an internal buffer.
...
* Trigger Configuration: The example uses Pin 5 (TRIG) and Pin 6 (GND) of the rear DIGITAL I/O port as the trigger input interface.
* Buffer & Speed: The scope mode stores up to 15000 samples with given averaging at max 100 kHz in an internal buffer. 
