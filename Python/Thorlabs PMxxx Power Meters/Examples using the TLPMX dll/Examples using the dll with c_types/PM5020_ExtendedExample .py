"""
Example Title: PM5020 ExtendedExample
Example Date of Creation(YYYY-MM-DD): 2023-07-26
Example Date of Last Modification on Github: 2026-10-20
Version of Python used for Testing and IDE: 3.10.0,3.13
Version of the Thorlabs SDK used: Thorlabs Optical Power Meter Version 7.0
==================
...
def ConnectDevice():
    ...
    try:
        lib.TLPMX_findRsrc(pmHandle, byref(deviceCount))
        lib.TLPMX_getRsrcName(pmHandle, 0, resourceName)
        lib.TLPMX_getRsrcInfo(pmHandle, 0, modelName, serialNumber, message, deviceAvailable)
        lib.TLPMX_close(pmHandle)
        pmHandle.value = 0
        lib.TLPMX_init(resourceName, c_bool(True), c_bool(True), byref(pmHandle))
    ...
    try:
        lib.TLPMX_getSensorInfo(pmHandle, message, message, message, sensorType1, message, message, TLPM_SENSOR_CHANNEL1)
    except:
        sensorType1.value = c_char_p(b'')._objects

    try:
        lib.TLPMX_getSensorInfo(pmHandle, message, message, message, sensorType2, message, message, TLPM_SENSOR_CHANNEL2)
    except:
        sensorType2.value = c_char_p(b'')._objects
...
def SetWavelength(setWavelength,CHANNEL):
    ...
    try:
        lib.TLPMX_setWavelength(pmHandle, c_double(setWavelength),CHANNEL)
        lib.TLPMX_getWavelength(pmHandle, TLPM_ATTR_SET_VAL,byref(wavelength),CHANNEL)
    except:
...
def SetRange(sensorType,setRange,setPowerAutoRange,CHANNEL):
    if sensorType == "power":
        if setPowerAutoRange == True:
            err = lib.TLPMX_setPowerAutoRange(pmHandle, True,CHANNEL)
            if CHANNEL == TLPM_SENSOR_CHANNEL1:
                print("Power Range for Channel 1 is set to AUTO")
            elif CHANNEL == TLPM_SENSOR_CHANNEL2:
                print("Power Range for Channel 2 is set to AUTO")
        else:
            lib.TLPMX_setPowerAutoRange(pmHandle, False,CHANNEL)
            powerMax = c_double(0)
            powerMin = c_double(0)
            range = c_double(0)
            lib.TLPMX_getPowerRange(pmHandle, TLPM_ATTR_MAX_VAL,byref(powerMax),CHANNEL)
            lib.TLPMX_getPowerRange(pmHandle, TLPM_ATTR_MIN_VAL,byref(powerMin),CHANNEL)
            ...
            lib.TLPMX_setPowerRange(pmHandle, c_double(setRange),CHANNEL)
            lib.TLPMX_getPowerRange(pmHandle, TLPM_ATTR_SET_VAL,byref(range),CHANNEL)
            if CHANNEL == TLPM_SENSOR_CHANNEL1:
        ...
        range = c_double(0)
        lib.TLPMX_getEnergyRange(pmHandle, TLPM_ATTR_MAX_VAL,byref(energyMax),CHANNEL)
        lib.TLPMX_getEnergyRange(pmHandle, TLPM_ATTR_MIN_VAL,byref(energyMin),CHANNEL)
        ...
        lib.TLPMX_setEnergyRange(pmHandle, c_double(setRange),CHANNEL)
        lib.TLPMX_getEnergyRange(pmHandle, TLPM_ATTR_SET_VAL,byref(range),CHANNEL)
...
def GetValue(sensorType,CHANNEL):
    ...
    while True:
        if sensorType == "power":
            power =  c_double()
            lib.TLPMX_measPower(pmHandle, byref(power), CHANNEL)
            value[0][1] = power.value
        elif sensorType == "energy":
            energy =  c_double()
            try:
                lib.TLPMX_measEnergy(pmHandle, byref(energy), CHANNEL)
            except:
...
def GetValueDual(sensorTypeCH1,sensorTypeCH2):
    ...
    while True:
        if sensorTypeCH1 == "power":
            power =  c_double()
            lib.TLPMX_measPower(pmHandle, byref(power), TLPM_SENSOR_CHANNEL1)
            value[0][1] = power.value
        elif sensorTypeCH1 == "energy":
            energy =  c_double()
            try:
                lib.TLPMX_measEnergy(pmHandle, byref(energy), TLPM_SENSOR_CHANNEL1)
            except:
                #For consoles except PM5020, if the energy fails to fetch, it will return timeout error
                #if the console is PM5020, it will keep waiting until the energy is fetched
                pass
        ...
        if sensorTypeCH2 == "power":
            power =  c_double()
            lib.TLPMX_measPower(pmHandle, byref(power), TLPM_SENSOR_CHANNEL2)
            value[0][2] = power.value
        elif sensorTypeCH2 == "energy":
            energy =  c_double()
            try:
                lib.TLPMX_measEnergy(pmHandle, byref(energy), TLPM_SENSOR_CHANNEL2)
            except:
                pass
            finally:
