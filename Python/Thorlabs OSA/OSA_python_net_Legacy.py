...
clr.AddReference("C:\\Program Files\\Thorlabs\\Thorlabs OSA\\ThorlabsOSAWrapper.dll")
...
def OnSingleAcquisition(sender, event_args):
    if event_args.UpdateFlag == AcquisitionUpdateFlag.Spectrum:
        print('Spectrum Acquired')
        spectrum = SpectrumStruct(sender.GetLastSpectrumLength(), True)
        sender.GetLastSpectrum(spectrum)
...
def main():
    ...
    if DeviceLocator.InitializeSpectrometers() > 0: 
